...
class PPO(BatchREINFORCE):
    ...
    def ppo_surrogate(self):
        ...
        lrmult = tfu.get_placeholder(name="lrmult",
                                     dtype=tf.float32,
                                     shape=[])
        self.clip_coef = self.clip_coef * lrmult
        ratio = tf.exp(self.pi.pd.logp(actions) - self.old_pi.pd.logp(actions))
        ratio_clip = tf.clip_by_value(ratio, 1 - self.clip_coef, 1 + self.clip_coef)
        surr = tf.reduce_mean(tf.minimum(ratio * adv, ratio_clip * adv))
        optim = tf.train.AdamOptimizer(self.learn_rate).minimize(-tf.reduce_mean(surr),
    ...
    def train_from_paths(self, paths, nsamples=None):
        ...
        # Schedule for clip_param
        # --------------------------

        if self.schedule == 'linear' and nsamples is not None:
            self.cur_lrmult = max(1.0 - float(self.tot_samples) / nsamples, 0)
        ...
        for ep in range(self.epochs):
            for mb in range(int(num_samples / self.mb_size)):
                ...
                self.ppo_optim(obs, act, adv, self.cur_lrmult)
